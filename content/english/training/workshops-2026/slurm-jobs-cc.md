---
weight: 1500
linkTitle: "Running jobs on CC"
Title: "Running jobs on CC"
...
{{< highlight bash >}}
which lmp
/cvmfs/soft.computecanada.ca/easybuild/software/2023/x86-64-v4/MPI/intel2023/openmpi4/lammps-omp/20240829/bin/lmp
{{< /highlight >}}
...
{{< highlight bash >}}
sbatch --account=def-sponsor2 mc-sleep-job.sh
{{< /highlight >}}
...
{{< highlight bash >}}
sbatch mc-runlmp-1cpu-serial.sh
{{< /highlight >}}
...
{{< alert type="info" >}}
The last job with 16 CPUs will fail because on MC, the maximum physical cores is 12. The same scipt should work on national clusters.
{{< /alert >}}
...
{{< collapsible title="Example of a job script to LAMMPS using apptainer." >}}
{{< highlight bash >}}
#!/bin/bash
...
{{< highlight bash >}}
sbatch --time=1:00:00 mc-runlmp-1cpu-serial.sh
{{< /highlight >}}
...
{{< highlight bash >}}
sbatch --mem=3000M mc-runlmp-1cpu-serial.sh
{{< /highlight >}}
...
{{< highlight bash >}}
cat mc-runlmp-1cpu-jobarray.sh
{{< /highlight >}}

**Now, submit the job using:**

{{< highlight bash >}}
sbatch mc-runlmp-1cpu-jobarray.sh
{{< /highlight >}}
...
sbatch --array=1,3 mc-runlmp-1cpu-jobarray.sh
{{< /highlight >}}
...
cat multiple-dir/mc-run-glost.sh
{{< /highlight >}} 
